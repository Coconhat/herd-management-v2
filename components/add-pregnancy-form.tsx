...
export function AddPregnancyForm({
  cows,
  breedingRecords,
}: {
  cows: Cow[];
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const expectedCalvingDate =
        (formData.get("expected_calving_date") as string) ||
        calculateExpectedCalvingDate(conceptionDate);

      const pregnancyData = {
        user_id: user.id,
        cow_id: formData.get("cow_id") as string,
        breeding_record_id:
          (formData.get("breeding_record_id") as string) || null,
      ...
      setError(
        error.message || "An error occurred while adding the pregnancy record"
      );
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-green-600">
                Leave empty to auto-calculate (283 days from conception)
              </p>
            </div>
          </div>
